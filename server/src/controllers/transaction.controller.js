...
export const createTransaction = async (req, res) => {
  try {
    const { amount, type, category_id, description, transaction_timestamp } =
      req.body;

    if (!amount || !type || !transaction_timestamp)
      throw ApiError.badRequest(
        "amount, type, and transaction_timestamp are required.",
      );
    ...
    if (category_id !== undefined && category_id !== null) {
      ...
      if (!categoryMeta) throw ApiError.notFound("Category not found.");
    }
    ...
    console.log(
      `[TRANSACTION - ${time}] User ${req.user.id} created transaction ${rows[0].id}`,
    );
...
export const getTransactions = async (req, res) => {
  try {
    ...
    if (category_id) {
      ...
      if (!catCheck[0]) throw ApiError.badRequest("Invalid category_id.");
...
export const exportTransactions = async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="mind-wallet-${userId}.xlsx"`,
    );
...
export const updateTransaction = async (req, res) => {
  try {
    ...
    const { amount, category_id, description, transaction_timestamp } =
      req.body;
    ...
    if (
      ...
      transaction_timestamp === undefined
    )
      throw ApiError.badRequest(
...
export const createTransactionRecord = async (
  userId,
  { amount, transactionType, category, description, timestamp },
) => {
  let categoryId = null;
  if (category) {
    const { rows: catRows } = await db.query(
...
export const deleteTransaction = async (req, res) => {
  try {
    ...
    const time = new Date().toLocaleTimeString("tr-TR", { hour12: false });
    console.log(
      `[TRANSACTION - ${time}] User ${req.user.id} deleted transaction ${parsedId}`,
    );

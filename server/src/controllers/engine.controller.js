...
export const action = async (req, res) => {
    try {
        ...
        if (buttonPayload.action === 'confirm_transaction') {
            ...
            return quickReply(res, `İşlem eklendi! ${tx.amount.toLocaleString('tr-TR')} TL ${toTR(tx.category ?? '')} kaydedildi.`, [
                { id: 'end_transaction', label: 'Evet, işlem ekle', payload: { action: 'start_transaction' } },
                { id: 'end_analyze', label: 'Aylık Özet', payload: { action: 'start_analysis' } },
                { id: 'end_done', label: 'Hayır, bitir', payload: { action: 'done' } },
            ]);
        }
        ...
        if (buttonPayload.action === 'confirm_goal_contribution') {
            ...
            invalidateContext(userId);
            const g = rows[0];
            return quickReply(res, `${Number(contribution.amount).toLocaleString('tr-TR')} TL "${g.title}" hedefine eklendi! Toplam: ${Number(g.current_amount).toLocaleString('tr-TR')} / ${Number(g.target_amount).toLocaleString('tr-TR')} TL.`, [
                { id: 'end_analyze', label: 'Aylık Özet', payload: { action: 'start_analysis' } },
                { id: 'end_transaction', label: 'İşlem Ekle', payload: { action: 'start_transaction' } },
                { id: 'end_done', label: 'Hayır, teşekkürler', payload: { action: 'done' } },
            ]);
        ...
        if (buttonPayload.action === 'confirm_pledge') {
            ...
            return quickReply(res, `Söz verildi! ${Number(pledge.amount).toLocaleString('tr-TR')} TL'yi "${pledge.goalTitle}" hedefin için ayırdım. Bu ay bu kategoride gerçekten az harcarsan, tutarı otomatik hedefe aktaracağım.`, [
                { id: 'end_analyze', label: 'Aylık Özet', payload: { action: 'start_analysis' } },
                { id: 'end_transaction', label: 'İşlem Ekle', payload: { action: 'start_transaction' } },
                { id: 'end_done', label: 'Hayır, teşekkürler', payload: { action: 'done' } },
            ]);
        ...
        if (buttonPayload.action === 'confirm_routing') {
            ...
            invalidateContext(userId);
            const g = rows[0];
            return quickReply(res, `${Number(route.amount).toLocaleString('tr-TR')} TL, ${g.title} hedefine yönlendirildi. Toplam: ${Number(g.current_amount).toLocaleString('tr-TR')} / ${Number(g.target_amount).toLocaleString('tr-TR')} TL.`, [
                { id: 'end_analyze', label: 'Aylık Özet', payload: { action: 'start_analysis' } },
                { id: 'end_transaction', label: 'İşlem Ekle', payload: { action: 'start_transaction' } },
                { id: 'end_done', label: 'Hayır, teşekkürler', payload: { action: 'done' } },
            ]);

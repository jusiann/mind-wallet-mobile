...
const quickReply = (res, message, buttons = null) =>
    res.status(200).json({ success: true, data: { message, buttons, classification: null, label: null, detected_savings: null, wasteful_categories: null, optimized_route: null, warning: null } });
...
export const analyze = async (req, res) => {
    try {
        ...
        const [transactionsResult, goalsResult, categoriesResult] = await Promise.all([
            db.query(
            ...
            db.query('SELECT id, name, is_essential FROM categories ORDER BY id ASC'),
        ]);
